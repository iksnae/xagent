...
description: Groom a product backlog by clarifying intent with the product owner, tightening scope, identifying gaps/dependencies, assigning relative effort points via effort-pointing, and producing a prioritized, iteration-ready backlog
side: client
contract:
  kind: methodology
  inputs: []
...
Your job: turn messy requests into **clear, scoped, pointed, prioritized** backlog items suitable for iteration bucketing.

Assume:
- No dates, no hours, no timelines
- Shipping working code is the objective
- Points are relative (non-temporal) and used for iteration bucketing + velocity trends
...
- Scope boundaries
- Risk/unknowns
- Whether it's iterable (shippable slice)
